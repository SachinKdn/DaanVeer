...
public class DonationSkeleton extends AppCompatActivity {
     ActivityDonationSkeletonBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding=ActivityDonationSkeletonBinding.inflate(getLayoutInflater());
        ...
        database.getReference().child("Donation").child(donatorId).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if(donation.getStatus().equals("Completed")) {
                    binding.btnReject.setVisibility(View.GONE);
...
//                Log.d("SachinKadian","sadasd" + snapshot.getKey());
        ...
        database.getReference().child("Organisation").child(auth.getUid()).child("orgName").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                orgName = snapshot.getValue(String.class);
            }
        ...
        binding.btnAccept.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Toast.makeText(DonationSkeleton.this, "Your request is accepted", Toast.LENGTH_SHORT).show();
                database.getReference().child("Donation").child(donatorId).child("organinsation").setValue(auth.getUid());
                database.getReference().child("UsersDonation").child(donatorId).child(newest).child("organinsation").setValue(auth.getUid());

                database.getReference().child("Donation").child(donatorId).child("orgName").setValue(orgName);
                database.getReference().child("UsersDonation").child(donatorId).child(newest).child("orgName").setValue(orgName);

                database.getReference().child("Donation").child(donatorId).child("status").setValue("In Progress");
                database.getReference().child("UsersDonation").child(donatorId).child(newest).child("status").setValue("In Progress");
        ...
        binding.btnReject.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Toast.makeText(DonationSkeleton.this, "Your request is Rejected", Toast.LENGTH_SHORT).show();
            }
        });
